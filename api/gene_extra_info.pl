...
sub createCoreTables {
	my ($gene, $exon, $dblink, $translation, %uniq, $gene_adaptor, $transcript_adaptor, $translation_adaptor, $probefeature_adaptor, $probeset_adaptor);
	...
	$probeset_adaptor = Bio::EnsEMBL::Registry->get_adaptor($species, "funcgen", "ProbeSet");
...
#			foreach my $dblink(@{ $trans->get_all_DBLinks }) {
			...
			## MICROARRYAYS
			if(defined $probeset_adaptor) {
				my @probesets = @{$probeset_adaptor->fetch_all_by_transcript_stable_id($trans->stable_id)};
